...
tileSize      = 10		# height/width of mosaic tiles in pixels
tileMatchsize = 6		# tile matching resolution (higher values give better fit but require more processing)
magnify    = 3		# the image will be this many times wider and taller than the original

tBlocksize = tileSize / max(min(tileMatchsize, tileSize), 1)
WORKER_COUNT = max(cpu_count() - 2,2)
OUT_FILE = 'mosaic.jpg'
EOQ_VALUE = None
...
class TargetImage:
	...
	def get_data(self):
		...
		w = img.size[0] * magnify
		h = img.size[1]	* magnify
		...
		wtblock = int(w/tBlocksize)
		htblock = int(h/tBlocksize)
		small_img = large_img.resize((wtblock, htblock), Image.ANTIALIAS)
...
class TileProcessor:
	...
	def __process_tile(self, tile_path):
		try:
			...
			w1 = int((w - min_dimension) / 2)
			h1 = int((h - min_dimension) / 2)
			w_crop = w1
			h_crop = h1
			img = img.crop((w_crop, h_crop, w - w_crop, h - h_crop))

			Tblocksize =int(tileSize/tBlocksize)

			large_tile_img = img.resize((tileSize, tileSize), Image.ANTIALIAS)
			small_tile_img = img.resize((Tblocksize, Tblocksize), Image.ANTIALIAS)
...
class MosaicImage:
	def __init__(self, original_img):
		...
		self.x_tile_count = original_img.size[0] / tileSize
		self.y_tile_count = original_img.size[1] / tileSize
	...
	try:
		...
		for x in range(mosaic.x_tile_count):
			for y in range(mosaic.y_tile_count):
				large_box = (x * tileSize, y * tileSize, (x + 1) * tileSize, (y + 1) * tileSize)
				small_box = (x * tileSize/tBlocksize, y * tileSize/tBlocksize, (x + 1) * tileSize/tBlocksize, (y + 1) * tileSize/tBlocksize)
...
if __name__ == '__main__':
	if len(sys.argv) < 3:
		print ('Please Use -> %s <image> <tiles directory>\r' % (sys.argv[0],))
	else:
